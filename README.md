...
데이터 링크 계층의 이더넷 규칙으로는 같은 네트워크로 데이터를 전송할 수는 있으나 다른 네트어크로는 데이터를 전송할 수 없습니다.
...
**C 클래스는 처음 24비트가 네트워크 ID, 다음 8비트가 호스트 ID**입니다. 공유기가 만들어내는 사설 IP가 여기에 속합니다.
...
하지만 IP 주소는 외우기 복잡하고 사용하기 불편해서 DNS를 사용합니다.
